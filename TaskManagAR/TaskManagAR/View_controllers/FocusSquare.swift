...
class FocusSquare: SCNNode {
    ...
    private func scaleBasedOnDistance(camera: ARCamera?) -> Float {
        if let camera = camera {
            ...
            let distanceFromCamera = GLKVector3Distance(GLKVector3Make(self.worldPosition.x, self.worldPosition.y, self.worldPosition.z), GLKVector3Make(SCNVector3.positionFromTransform(camera.transform).x, SCNVector3.positionFromTransform(camera.transform).y, SCNVector3.positionFromTransform(camera.transform).z))
            //simd_distance(self.worldPosition, SCNVector3.positionFromTransform(camera.transform))
